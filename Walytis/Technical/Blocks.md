...
This is a wall-clock timestamp generated by the creating node during the process of creation, provided by the OS' clock in UTC.
This field is purely informative. Walytis make no guarantees about a block's creation time, and it is as easy as can be to fake at the time of block creation.

Until Walytis-Core version 2.4.17, errors would be raised when nodes received blocks with creation times in their wall-clock's future, or when blocks had older timestamps than their parents.
This strictness has been removed to remove the necessity for nodes on the network to maintain well-synchronised clocks.

This means that Walytis' block-chaining-chronology is completely independent of wall-clock time-keeping.

### Creator ID

This is the IPFS peer ID of the node which created the block.
Like the creation time field, this is purely informative, and easy as can be to fake at the time of a block's creation.
